...
3. Expand source/detail search to query transcript/article chunks, not just
   ...
   Timestamped chunks inside source detail can also jump local playback to
   their indexed moment.
...
6. Add the first agent search tool with cited results and action previews.
   ...
   undo support. `DeterministicToolAgentRuntime` now runs over those tools to
   produce cited local answer payloads, giving the app a working runtime
   scaffold before a cloud/local model provider is selected.
